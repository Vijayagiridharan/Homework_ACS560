...
/**
 * CustomExceptionHandler is a class that handles exceptions thrown by
...
public class CustomExceptionHandler {
     ...
     * @return a ResponseEntity containing the error message and HTTP status 404 (Not Found)
     */
    @ExceptionHandler(CustomNotFoundException.class)
    @ResponseStatus(HttpStatus.NOT_FOUND)
    public ResponseEntity<Map<String, String>> handleNotFoundException(CustomNotFoundException ex) {
        Map<String, String> response = new HashMap<>();
        response.put("error", ex.getMessage());
